    ...
    <div class="container pt-24 md:pt-36 mx-auto flex flex-wrap flex-col md:flex-row items-center">
      ...
      <div class="flex flex-col w-full xl:w-2/5 justify-center lg:items-start overflow-y-hidden ml-24">
        ...
        <p class="leading-normal text-base md:text-2xl mb-8 text-center md:text-left">
          Masuk ke akun Anda untuk mengakses semua fitur dan informasi terbaru.
        </p>
        ...
        <form method="POST" action="{{ route('login') }}" class="bg-gray-900 opacity-75 w-full shadow-lg rounded-lg px-8 pt-6 pb-8 mb-4">
          ...
          <div class="mb-4">
            ...
            <input type="password" class="shadow appearance-none border rounded w-full p-3 text-gray-700 leading-tight focus:ring transform transition hover:scale-105 duration-300 ease-in-out" id="password" name="password" type="text" placeholder="Masukkan password anda" />
            @error('password')
            <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
          ...
          <label class="text-white mt-3">Belum Punya akun?</label>
          <a href="{{ route('register') }}" class="bg-gradient-to-r text-blue-400 transform transition hover:scale-105 duration-300 ease-in-out" type="submit">
            Daftar
          </a>
        </form>
